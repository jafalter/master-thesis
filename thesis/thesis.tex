...
\fbox{
    \parbox{\textwidth}{
        \procedure[linenumbering, syntaxhighlight=auto]{$GEN()$} {
            k \sample \ZZ_q \\
            r \sample \ZZ_q \\
            \pcreturn (k, r)
        }
        \procedure[linenumbering, syntaxhighlight=auto]{$GEN\_PART\_SIG(M, k, r, g^{k'}, g^{r'}$)} {
            e = h (M || g^{k} + g^{k'} || g^{r} + g^{r'}) \\
            sig\_part = k + e * r \\
            \pcreturn (sig_part, g^{k}, g^{r'})
        }\\
        \procedure[linenumbering, syntaxhighlight=auto]{$VERF\_PART\_SIG(M, k, r, g^{k'}, g^{r'}, sig\_part$)} {
            e = h (M || g^{k} + g^{k'} || g^{r} + g^{r'}) \\
            \pcreturn g^{sig\_part} = g^{k'} + g^{e * r}
        }\\
        \procedure[linenumbering, syntaxhighlight=auto]{$FINALIZE\_SIG(sig\_part, sig\_part', g^{k}, g^{k'})$)} {
            \pcreturn (sig\_part + sig\_part', g^{k} + g^{k'})
        }
    }
}
...
\fbox{
\parbox{\textwidth} {
    \procedure[linenumbering, syntaxhighlight=auto]{$ADAPT\_PART\_SIG(sig\_part, x)$} {
        sig\_part\_apt = sig_part + x \\
        \pcreturn (sig\_part\_apt, g^{x})
    }
    \procedure[linenumbering, syntaxhighlight=auto]{$EXT\_WIT(sig\_final, sig\_part, sig\_part\_apt')$}{
        sig\_part' = sig\_final - sig\_part \\
        x = sig\_part\_apt' - sig\_part ' \\
        \pcreturn (x)
    }\\
    \procedure[linenumbering, syntaxhighlight=auto]{$VERF\_APT\_SIG(M, k, r, g^{k'}, g^{r'}, g^{x}, sig\_part\_apt')$} {
        e = h (M || g^{k} + g^{k'} || g^{r} + g^{r'}) \\
        \pcreturn g^{sig\_part\_apt'} = g^{k'} + g^{e * r} + g^{x}
    }
}
}
