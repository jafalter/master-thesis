...
Additionally a transaction consists of a version number, and a locktime field which can be used, to only allow the transaction to become valid after a certain block has been mined. Figure \ref{fig:btc-tx} shows a decoded Bitcoin transaction

\begin{definition}[Bitcoin Transaction]
    A Bitcoin transaction consists of a series of input UTXOs $\varInputs$, a series of output UTXOs $\varOutputs$, a transaction version $\varVersion$ and a locktime $\varTime$:
