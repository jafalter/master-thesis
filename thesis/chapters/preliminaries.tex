...
generators in $\cnstGroup$, whereas adjacent means the discrete logarithm of $\varH$ in regards to $\varG$ is not known. Exponention stands for repeated application of the group operation. For two scalars
$\varScA$ and $\varScB$ multiplication is defined as $\varScA \opTimesScalar \varScB$. We define addition between two curve points as
$\funGen{\varScA} \opAddPoint \funGen{\funGen{\varScB}} \opEq \funGen{\varScA \opTimesScalar \varScB}$.
...
\begin{itemize}
    \item For two adjacent elliptic curve generators $\varG$ and $\varH$ a coin in Mimblewimble is of the form $\varCoin \opAssign \funGen{\varValue} \opAddPoint \funGenH{\varNonce} \opSeperate \varProof$. $\varCoin$ is a so called Pedersen Commitment \cite{pedersen1991non}
