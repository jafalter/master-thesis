...
A UTXO is a output constructed in a previous transaction which holds value in the form of an amount expressed in Bitcoin (more precisely in Satoshis, which is the smallest unit of Bitcoin) and a locking condition (referred to as scriptPubKey).
...
\begin{definition}[$\varUTXO$] A unspent transaction output ($\varUTXO$) is a data structure consisting of a locking condition $\varScriptPubKey$, a value expressed
    in Bitcoin satoshis $\varValue$ and an unlocking script $\varScriptSig$ which is initially empty and has to be provided by the owner when spending
    the UTXO in a transaction.
    \[ \varUTXO \opAssign \{ \varValue \opSeperate \varScriptPubKey \opSeperate \varScriptSig \} \]
\end{definition}

We define two auxiliary functions for the creation and spending of an $\varUTXO$.

\begin{center}
    \fbox{
    \begin{varwidth}{\textwidth}
        \procedure[linenumbering]{$\procCreateUTXO{\varValue}{\varScriptPubKey}$} {
        \pcreturn \{ \varValue \opAssign \varValue, \varScriptPubKey \opAssign \varScriptPubKey, \varScriptSig \opAssign \cnstEmptySet \}
        }
        \procedure[linenumbering]{$\procSpendUTXO{\varUTXO}{\varScriptSig}$} {
        \{ \varValue, \varScriptPubKey \} \opFunResult \varUTXO \\
        \pcreturn \{ \varValue \opAssign \varValue, \varScriptPubKey \opAssign \varScriptPubKey, \varScriptSig \opAssign \varScriptSig \}
        }
    \end{varwidth}
    }
\end{center}
...
Additionally a transaction consists of a version number, and a locktime field which can be used, to only allow the transaction to become valid after a certain block has been mined.
Figure~\ref{fig:btc-tx} shows a decoded Bitcoin transaction

\begin{definition}[Bitcoin Transaction]
    A Bitcoin transaction consists of a series of input UTXOs $\varInputs$, a series of output UTXOs $\varOutputs$, a transaction version $\varVersion$, and a optional locktime $\varTime$:
    \[ \varBtxtx \opAssign \{ \varVersion, \varTime, \varInputs, \varOutputs \} \]
\end{definition}
...
\begin{itemize}
    ...
    \item Every spend input contains a $\varScriptSig$ which unlocks the respective $\varScriptPubKey$.
...
\subsubsection{P2PK, P2PKH - Transaction \label{subsubsec:p2pk}}

P2PK stands for Pay-to-Public-Key and P2PKH for Pay-to-Public-Key-Hash.
In this type of output the $\varScriptPubKey$ will be constructed such that its value unlocks if a correct signature is provided in the $\varScriptSig$ for a corresponding public key $\varScriptPubKey$.
P2PKH is an update to this script in which the $\varScriptPubKey$ contains a hased version of the public key, not the public key itself.
To spend a P2PKH output one has to additionally provide the unhashed public key in addition to a valid signature.
...
Delgado et at. found in their paper Analysis of the Bitcoin UTXO set from 2017 that more then 80\% of the UTXO set at that time consisted of P2PHK transactions, whereas about 17\% were P2SH and 0.12\% P2PK outputs. ~\cite{delgado2018analysis}
...
\subsubsection{P2SH - Transaction \label{subsubsec:p2sh}}

If more advanced spending conditions, such as multi signature are required, P2SH, introduced in 2012, is a way to implement those in a space efficient and simple matter.
Here the locking condition $\varScriptPubKey$ does not contain a script, but instead the hash of a script.
Upon spending the spender has to provide the original script which when hashed will equal the hash given in the locking condition.
If the hashes matches the script itself will be executed, requiring the spender to additionally provide the unlocking requirements for the script itself.
The advantages of using this approach over just handcrafting custom locking script is that the locking scripts are rather short making the transactions smaller and therefore reducing fees, or rather shifting the fees from the party sending funds to the output, to owner of the output who has to provide the full script when spending it again.
Additionally this type of output can be encoded again into a Bitcoin address similiar to a P2PKH output, making it easy to request a payment.

\begin{definition}[Bitcion - Transaction Scheme]
    $\varTx \opFunResult \procBuildTransaction{\varInputs}{\varOutputs}{\varVersion}{\varTime}$
\end{definition}
