...
For the correctness of the distributed key-generation protocol $\procKeyGenPtId$, special care needs to be taken to gurantee a uniformly random distribution of generated keys as pointed out by Lindell Yehuda in~\cite{lindell2017fast}.
...
\begin{definition}[Two Party Fixed Witness Adaptor Schnorr Signature Scheme]
    ...
    From the ~\cref{def:sig:two-party-sig}, we now derive an adapted signature scheme $\varSigSchemeApt$, which allows one of the participants to hide the discrete logarithm $\varWit$ of a statement $\varStatement \opAssign \funGen{\varWit}$ chosen
...
Note that our definition of the adaptor signature scheme is different from the definition seen in~\cref{sec:pre:aptsignatures}.
