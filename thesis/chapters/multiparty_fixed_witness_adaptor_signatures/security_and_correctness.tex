...
\begin{proof}
    ...
    $(\varStatement \opSeperate \varWit) \opIn \cnstRelation$ (whereas \todo{whereas does not mean what you want to see here. I think you may use where here instead. Same problem appeared already several times in the text} $\varWit$ is the output for the $\procExtWitId$ function) hold.
    ...
    \end{align}
\end{proof}
\todo[inline]{In the proof above, I think that $\varSApt$ in 4.14 should be $\varSAptBob$? }
...
If $\varStatement \opEqNoQ \funStar{\varStatement}$ and $\cnstRelation (\varWit, \varStatement)$\todo{Have you introduced the notation $\cnstRelation (\varWit, \varStatement)$ before? This is not standard I think} the functionality returns 1, otherwise 0.
More formally:
\[
...
We recall from~\ref{sec:pre:privacy:zeroknowlegde} that a secure zero-knowledge proof must fulfill Completeness, Soundness and Zero-Knowledge. \todo{Where are these three properties defined? Give a reference to where you present them in the thesis or the corresponding paper}
...
\todo[inline]{I think here only you know what operations are hidden behind the dots. A reader would need to find the corresponding figure of dSign and compare line by line. I think it would be more readable if you put the complete dSign and highlight in a certain color what instructions you have added and deleted here.}
...
\todo[inline]{I think that the theorem is missing the assumption that the hash function is modeled in the random 
oracle model, which you use in the case where Bob is corrupted.}
...
\begin{proof}
    ...
    We have to proof\todo{I think you need to write prove instead of proof here?} two cases, one in which Alice is the corrupted party and one in which Bob is the corrupted party.
