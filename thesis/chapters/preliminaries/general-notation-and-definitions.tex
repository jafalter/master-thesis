...
We write procedures as $\styleFunction{function(\cdot)}$ and protocols (executed between multiple parties) as $\styleProtocol{protocol(\cdot)}$ \todo[inline]{Visually, the notation for function and protocol are not easy to distinguish. Is it easy to change to make it more different?}

\todo[inline]{When you want to put a reference to where you get a definition from, I would just put it at the side of the name of the definition }
...
\begin{definition}[Discrete Logarithm]\label{def:pre:discretelog}
    We define the discrete logarithm in a group $\cnstGroup$ of a point $\varH$ as the \removepms{number}\newpms{integer} $\varN$ such that for the groups generator $\varG$ the following holds:
    \[ \funGen{\varM} \opEqNoQ \varH \] \todo{should be $\VarN$ instead of $\VarM$?}
    The discrete logarithm is a hard relation as defined in~\cref{def:pre:hard-relation}.
\end{definition}

\todo[inline]{The discrete logarithm is not a hard relation. The hard relation is given by the discrete logarithm assumption (also called discrete logarithm problem). That is, an adversary $A$ given the tuple $(g, h)$ such that $h = g^n$ can compute $n$ only with negligible probability.}
...
\todo[inline]{This definition is not complete. The adversary has access to signing oracle as well.}
...
\begin{definition}[Commitment Scheme]\label{def:pre:commitment}
	...
	\todo[inline]{I find confusing that you are using $n$ in $1^n$ as well as input for commit. }
    ...
    \begin{itemize}
       ...
       \varRandSpace_{\varPublicParam}$ and a Commitment output $\varCommitment \opAssign \procCommit{\varInput_{\varB}, \varNonce}$ the probabilty of the adversary choosing the correct $\varB$ out of $\{0,1\}$
        must not be higher then $\frac{1}{2} + \funNegl{\varProbability}$. \todo{What is $\varProbability$ here?}
    \end{itemize}
\end{definition}
...
So no $\varX$ is known such that $\varH \opEqNoQ \funGen{x}$.
    In practice, this is often achieved by hashing $\varG$ and using the hash output as $\varH$. \todo{Mention that this is a special hash function that outputs a group element at random instead of a random value. }

\end{definition}



\todo[inline]{Here you are switching the topic a little, going from primitives to your security analysis technique. I would introduce this change in text or even create another subsection}
