...
One can argue that Transaction merging~(see \cref{sec:pre:mimblewimble:merge})\todo{putting the section without any parenthesis or context breaks the reading flow. What about adding (see section x.y)?} together with Cut-through~\cref{par:pre:mimblewimble:cut} should make transactions unlinkable.
...
\todo[inline]{Then what? why are you using Mimblewimble? Should we just not care about this as it is not privacy-preserving? (I am just asking this because could be one of the questions of the reviewer :P )}
...
By the rules of arithmetic,\todo{hehe it sounds really fun "by the rules of arithmetic". Perhaps, write: ``then it follows that...''} it then must also hold that
...
\todo[inline]{What does the symbol $\opSeperate$ means? You have not defined it before}
...
This problem has been mitigated in Cryptocurrencies implementing the protocol\todo{what protocol?} by including an additional variable $\varOffset$ in the Kernel, called offset value.
...
\end{figure}
\todo[inline]{In this figure, what is $x$ and where does it come from?}


\todo[inline]{The attack that you show below is known before? (if so, please put reference). Or did you come up with it? If so, then write that explicity in the text (maybe even make this a (sub)section on its own to highlight that.  }
