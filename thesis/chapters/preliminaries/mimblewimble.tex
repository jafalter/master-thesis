...
In this section we will outline the fundamental properties of the protocols employed in Mimblewimble which are relevant for the thesis and particularly the construction of the Atomic Swap protocol constructed in~\cref{ch:atomicswap}.
...
If $\varTx_0$ and $\varTx_1$ are valid as of~\cref{def:pre:tx-mw-validity} that means $\varCoinInp^0 \opSub \varCoinOut^0 \opSub \funGenH{\varSupply_0} \opEqNoQ \varExcess_0 \opSeperate \funList{\varProof_0}$ contains valid range proofs for the outputs
...
Now lets recall the equation given for transaction well-balancedness in~\cref{def:pre:tx-well-balancedness}:
