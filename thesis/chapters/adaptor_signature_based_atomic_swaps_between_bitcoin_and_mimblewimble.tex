...
First of all we define two protocols $\procDSignId$ and $\procDRProofId$, whereas $\procDSignId$ is a protocol between two-parties running the partial signature creation outlined in section \ref{sec:schnorrInst} and
$\procDRProofId$ is a two-party protocol to create a single proof $\varProof$ for a commitment $\varCommitment$ for which the blinding factor consists of two shares $(\varBlindingFactorAlice, \varBlindingFactorBob)$.

\begin{center}
    \fbox{
    \begin{varwidth}{\textwidth}
        \procedure[linenumbering]{$\procDSign{\varMsg}{\varSecKeyAlice}{\varSecKeyBob}$} {
        \pcreturn 1
        }
        \procedure[linenumbering]{$\procDRProof{\varCommitment}{\varValue}{\varBlindingFactorAlice}{\varBlindingFactorBob}$}{
        \pcreturn 1
        }
    \end{varwidth}
    }
...
\begin{figure}
    \begin{sideways}
        ...
        \< \< \t \pcreturn \cnstFalsum \\
        \< (\varSigBobCarol, \varPubKeyBobCarol, \funStar{\varSigContext}) \opFunResult \procDSign{\varMsg}{\varSecKeyBob}{\varSecKeyBob} \< \\
