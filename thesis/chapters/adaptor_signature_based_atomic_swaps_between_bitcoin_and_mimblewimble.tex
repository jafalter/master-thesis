...
\begin{definition}[Inflation Resistence]
    ...
    paramter $\varSecParam$ and a PPT adversary $\cnstAdversary$ the following holds:
...
We define a game $\procStealCoinId$ which takes as input a security paramter $\varSecParam$ and a coin $\varCoinInp$. A PPT adversary $\cnstAdversary$ is given the challenge to spend the input coin given in the parameter.
...
\begin{definition}[Theft-resistance]
    A Mimblewimble Transaction Scheme is called theft-resistent if for any input coin $\varCoinInp$, security parameter $\varSecParam$ and PPT adversary $\cnstAdversary$ the following holds:
    \[ \prob{\procStealCoin{\varSecParam}{\varCoinInp} \opEqNoQ 1 } \opSmEq \funNegl{\varN} \]
\end{definition}
...
sets depending on the bit $\varB$ chosen by the challenger and construct a transaction spending a coin with value $\varValue_b$ to and outputcoin $\varFundValue_b$ (as well as a change coin with the remainder).
...
\begin{definition}[Transaction indistinguishability]
    ...
    \[ \prob{\procTxInd{\varSecParam}{\varValueMax} \opEq 1} \opSmEq 0.5 \opAddScalar \funNegl{\varN} \]
\end{definition}
