...
\begin{itemize}
    \item $\varAddress \opFunResult \procLockAddr{\varPubKeyAlice}{\varStatement}{\varPubKeyBob}{\varTime}$: The locking address function lets Bob construct
    a Bitcoin address only spendable by Alice if she receives the discrete logrithm $\varWit$ of $\varStatement \opEqNoQ \funGen{\varWit}$.
    ...
    completed successfully. The function will create a lock output under the public key $\varPubKey \opAssign \varPubKeyAlice \opAddPoint \varStatement$ and return
    a Bitcoin address $\varAddress$. If Alice
    (knowing her secret key $\varSecKeyAlice$) acquires $\varWit$ with $\varStatement \opEqNoQ \funGen{\varWit}$ she can construct the complete secret key simply
    by calculating $\varSecKey \opAssign \varSecKeyAlice \opAddScalar \varWit$. This construction is similiar although much simpler to the locks described
    ...
    \item $\{ 1,0 \} \opFunResult \procVerifyLock{\varPubKeyAlice}{\varPubKeyBob}{\varStatement}{\varFundValue}$: The lock verification algorithm takes as input Alices',
    Bobs' public keys and the statement $\varStatement$. The function will recompute the address the lock address $\varAddress$ as created by
    $\procLockAddrId$ and then check the Bitcoin blockchain if the value locked under this address is indeed $\varFundValue$. Upon successfull verification
