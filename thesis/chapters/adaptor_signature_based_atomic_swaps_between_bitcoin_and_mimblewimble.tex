...
We start by proving Correctness for all three transaction schemes.
We then continue by showing that all three security definitions (Inflation-Resistence, Theft-Resistence, Transaction indistinguishability) hold for all 3 transaction types.
...
Correctness of the transaction scheme trivially follows from the correctness of the signature scheme used in the transaction protocol as well as the correctness of the range proof system, as those are the two verifications run by the $\procVerfTxId$ algorithm.
For the correctness of the range proof system we refer the reader again to the bulletproof paper~\cite{bunz2018bulletproofs}.
The instantiation of the Mimblewimble Transaction Scheme (defined in~\ref{def:atom:mw-tx-scheme} instantiated in figure~\ref{fig:inst-mw-tx}) uses a standard Two-Party Signature Scheme defined in~\ref{def:sig:two-party-sig} for which a correctness proof is available by Maxwell et al. in~\cite{maxwell2019simple}.

For the Extended Mimblewimble Transaction Scheme defined in~\ref{def:atom:ext-mw-tx-scheme} and instantiated in figure~\ref{fig:ext-mim-tx-spend},~\ref{fig:ext-mim-tx-recv} and~\ref{fig:ext-mim-tx-fin} the same signature scheme is used, therefore we can make the same argument as for the previous case.

For the Adapted Extended Mimblewimble Transaction Scheme which we have defined in~\ref{def:atom:apt-ext-mw-tx-scheme} and have shown a instantiation in figure~\ref{fig:inst-apt-mw-tx-recv} and~\ref{fig:inst-apt-mw-tx-fin} the Two Party Fixed Witness Adaptor Schnorr Signature Scheme which we have defined in~\ref{def:sig:two-party-fixed-wit-apt-sig} is used.
We have already shown correctness of this signature scheme in section~\ref{sec:sig:two-party-apt-security} by proofing that Adaptor Signature Correctness holds.
Furthermore the scheme used a multiparty version of the bulletproof system for which we refer the reader to~\cite{klinec2020privacy}.
