In this section we specify three protocols to build Mimblewimble transactions from the definitions found in~\cref{sec:atom:definitions}.
...
As input it takes a list of partial spendable coins $\funArray{\varPtSpendableCoinAlice}$, a transaction value $\varFundValue$ and an optional timelock $\varTime$ from the sender, and the other part of the shared spendable coins $\varPtSpendableCoinBob$ as well as the same transaction value $\varFundValue$ from the receiver.
...
\paragraph{A note on rogue-key attacks:} In~\cref{sec:sig:definitions} we mentioned that in a Two-Party Signature Scheme we need to take special care in the key generation phase not to be vulnerable against rogue-key attacks in which one of the parties public key is computed as a function of the other.
We see that in all of the protocols layed out in this section we do not take this special care, as the transaction receiver will always generate his private key after having seen the one by the sender.
This would in theory allow for the following rogue-key attack:
Imagine we have an attacker $\cnstAdversary$ who knows the value $\varValue$ of some coin $\varCoin \opEqNoQ \funGen{\varBlindingFactor} \opAddPoint \funGenH{\varValue}$. He would then compute $\varPubKeyAlice \opEqNoQ \varCoin \opAddPoint {(\funGenH{\varValue}})^{-1}$.
For the rogue-key attack to succeed $\cnstAdversary$ would now try to choose his pubkey as $\varPubKeyBob \opEqNoQ \varPubKeyAlice^{-1}$ such that it would cancel out Alice's key, however remembering the structure of Mimblewimble transactions we actually sign the Excess value $\varExcess \opEqNoQ \varInputs \opSub \varOutputs$, where $\varInputs$ and $\varOutputs$ is the list of input and output coins.
So in order to make the public keys cancel out $\cnstAdversary$ would have to choose his key as $\varPubKeyBob \opAssign \varPubKeyAlice$.
So then $\cnstAdversary$ would have to create a transaction spending Alice's coin to a pedersen commitment of the form $\varCoin \opEqNoQ \varPubKeyBob \opAddPoint \funGenH{\varValue}$.
In this case the Excess value would calculate like $\varPubKeyAlice \opAddPoint {\varPubKeyBob}^{-1}$ which by the definition of $\varPubKeyBob$ is $\varPubKeyAlice \opAddPoint {\varPubKeyAlice}^{-1}$ which would allow $\cnstAdversary$ to forge a signature.
However, since we chose $\varPubKeyBob$ as simply $\varPubKeyAlice$ and $\varPubKeyAlice \opEqNoQ \funGen{\varBlindingFactor}$ (from the original petersen commitment) the new output coin would in fact be identical to the input coin and the transaction would therefore simply spend a coin to itself.
In practice this kind of transaction is not allowed on the Grin or Bean network, therefore making this attack vector non-viable which is why our formalization is still secure.
Imagining it would not be disallowed for a moment, the only thing that $\cnstAdversary$ could achieve is to steal a fee from Alice.
As soon as he would try to add his own output coin the calculation of $\varExcess$ changes and the public keys would no longer cancel out.
