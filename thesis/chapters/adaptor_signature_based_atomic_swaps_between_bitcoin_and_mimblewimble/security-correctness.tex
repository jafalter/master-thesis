...
\textbf{Proof Idea}: We extend the protocol $\procDBuildMwTxId$ instantiated in section~\ref{sec:atom:protocols} with the following calls to the zero-knowledge proof of knowledge functionalities as depicted in~\ref{fig:atom:hybrid-dbuild}.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \end{enumerate}
        \item $\cnstSimulator$ is now able to construct $\funStarAlt{\funStarAlt{\varPreTx}}$ by again cloning $\varTx$ and setting $\funStarAlt{\funStarAlt{\varPreTx}}.\varSignature \opAssign {\varSigBob}_2$.
        The simulator will rewind the call to the TTP computing $\procDFinTxId$ and send $\funStarAlt{\funStarAlt{\varPreTx}}$ to $\cnstAdversary$ as if coming from Bob.
