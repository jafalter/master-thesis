...
\textbf{We extend the protocol $\procDSharedOutputMwTxId$} instantiated in section~\ref{sec:atom:protocols} with the following calls to the zero-knowledge proof of knowledge functionalities:
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item The simulator now needs to construct $\funStarAlt{\varPreTx}$ which should be returned to $\cnstAdversary$ calling $\procDRecvCoinsId$.
        The difficulty here is that Alice expects $\funStarAlt{\varPreTx}$ to be partially signed, meaning it has only the partial signature for the shared input coins in the signature field.
        $\cnstSimulator$ can calculate this signature as follows:
        \begin{gather}
            \varSigAlice \opFunResult \procSignPrt{\varTx.\varMsg}{\varSecKeyAlice}{\varNonceAlice}{\varTx.\varSigContext} \\
            \{ \varSAlice, \varRandAlice, \varSigContext \} \opFunResult \varSigAlice \\
            \{ \varS, \varRand \} \opFunResult \varTx.\varSignature \\
            \varSAliceBob \opAssign \varS \opSub \varSAlice \\
            \varRandAliceBob \opAssign \varRand \opAddPoint \varRandAlice^{-1} \\
            \varSigAliceBob \opAssign \{ \varSAliceBob, \varRandAliceBob, \varSigContext \} \\
        \end{gather}
        After having done this calculation $\cnstSimulator$ puts $\varSigAliceBob$ to the signature field of $\varTx$ and sends it as output to $\cnstAdversary$ (together with $\varPtSpendableCoinAlice$) when he calls $\procDRecvCoinsId$.
        $\cnstSimulator$ further externally sends $\cnstContinue$ to the TTP such that $\cnstAdversary$ receives his outputs in the ideal model.
        \item Eventually $\cnstAdversary$ sends $\varTx$ after which $\cnstSimulator$ outputs whatever $\cnstAdversary$ outputs.
    \end{enumerate}

    We claim that the simulation is perfect.
    Indeed after having successfully extracted the inputs of $\cnstAdversary$ and receiving the outputs from the TTP the only difficulty lies in constructing $\funStarAlt{\varPreTx}$ which is returned by $\procDRecvCoinsId$ in a real execution.
    This is achieved by the calculation we have layed out to construct $\varSigAliceBob$ from $\varTx.\varSignature$ and $\varSigAlice$ (which $\cnstSimulator$ can compute by himself knowing $\varSecKeyAlice$ and $\varNonceAlice$).
    Knowing $\varSigAliceBob$ $\cnstSimulator$ also knows what $\funStarAlt{\varPreTx}$ must be in a real execution, making the transcript identical.
    Also if the adversary cheats (sending an invalid $\varPreTx$) this is noticed by the simulator who then outputs $\cnstFalsum$ which is identical to a real execution.
    ...
    \begin{enumerate}
        ...
        \item Now the next challenge for $\cnstSimulator$ is that $\cnstAdversary$ expects a $\funStarAlt{\varPreTx}$ which is partially signed (i.e. has the partial signature from Alice + Bob shared output), but not yet the signature from Alice.
        The simulator can construct the updated pre-transaction as follows:
        $\cnstAdversary$ expects Alice to provide a partial signature under $\funGen{\varSecKeyAlice}$ with nonce commitment $\funGen{\varNonceAlice}$, as this is how $\cnstSimulator$ setup the signature context in step 2.
        Consequently the partial signature expected in $\funStarAlt{\varPreTx}$ has to be valid under $\varPubKeyAliceBob \opEqNoQ \varPubKey \opAddPoint {\funGen{\varSecKeyAlice}}^{-1}$ with nonce commitment $\varRandAliceBob \opEqNoQ \varRand \opAddPoint {\funGen{\varNonceAlice}}^{-1}$ with $(\varPubKey, \varRand)$ being the final composite values from $\varTx.\varSigContext$.
        The simulator can construct such a signature like follows:
        \begin{gather*}
            \{ \varS, \varRand \} \opFunResult \varTx.\varSignature \\
            \varSigAlice \opFunResult \procSignPrt{\varTx.\varMsg}{\varSecKeyAlice}{\varNonceAlice}{\varTx.\varSigContext} \\
            \{ \varSAlice, \varRandAlice, \varSigContext \} \opFunResult \varSigAlice \\
            \varSAliceBob \opAssign \varS \opSub \varSAlice \\
            \varSigAliceBob \opAssign \{ \varSAliceBob, \varRand \opAddPoint \varRandAlice^{-1}, \varSigContext \}
        \end{gather*}
        Now $\cnstSimulator$ constructs $\funStarAlt{\varPreTx}$ simply by cloning $\varTx$ and replacing $\funStarAlt{\varPreTx}.\varSignature$ with $\varSigAliceBob$.
        Knowing $\funStar{\varPtSpendableCoinBob}$ from the TTP output the simulator now simply sends ($\funStarAlt{\varPreTx}$, $\funStar{\varPtSpendableCoinBob}$) as output when $\cnstAdversary$ calls $\procDRecvCoinsId$.
        \item Finally $\cnstSimulator$ sends $\varTx$ as if coming from Alice and outputs whatever $\cnstAdversary$ outputs.
    \end{enumerate}

    We can easily claim that $\varTx$ sent by $\cnstSimulator$ in the last step is identical to the value from a real execution.
    ...
    Therefore the only thing that remains to show is that $\varPreTx$ and $\funStarAlt{\varPreTx}$ are indistinguishable from values exchanged in a real transcript.
    We note that $\varSecKeyAlice$ and $\varNonceAlice$ are uniformly distributed values in $\cnstIntegersPrimeWithoutZero{\varPrime}$.
    Consequently $\funGen{\varSecKeyAlice}$ and $\funGen{\varNonceAlice}$ are uniformly distributed in $\cnstGroup$.
    It is trivial to see that this must also hold for the values sampled by the simulator.
    Therefore the signature context constructed in step 2 for $\varPreTx$ must be indistinguishable from a real one, which also means that the $\varPreTx$ is indistinguishable.
    For $\funStarAlt{\varPreTx}$ to be indistinguishable it must hold that it completes into $\varSignature$ from the final transaction by adding the partial signature under $\varPubKeyAlice$ with nonce commitment $\varRandAlice$.
    It is easy to see that by the construction of $\varSigAliceBob$ in step 4 this must hold and therefore $\funStarAlt{\varPreTx}$ is indeed indistinguishable from a $\funStarAlt{\varPreTx}$ which would be sent in a real execution.
...
\textbf{Hybrid functionalities}: For this proof is enough to give the parties access to a trusted third party computing the $\procDSendCoinsId$ and the $\procDFinTxId$ protocol.
Further calls to a zero-knowledge proof of knowledge functionality are not needed.
This means that do not have to extend to original protocol instantiation.
...
\begin{proof}
    We proof security by construction of simulator $\cnstSimulator$ with access to a trusted third party (TTP) computing $\procDSharedInpMwTxId$ in the ideal model upon receiving inputs from the two parties.
    The simulators taks is to extract the adversaries inputs, send them to the TTp and construct a protocol transcript indistinguishable from a real one.
    ...
    \begin{enumerate}
        ...
        \item $\cnstSimulator$ externally sends the inputs ($\funArray{\varPtSpendableCoinAlice}$, $\varFundValue$, $\varTime$) to the TTP and receives ($\varTx$, $\funStar{\varSpendableCoinAlice}$).
        He further calculates $\varPreTx$ by cloning $\varTx$ and removing the last output coin (and proof) from $\varPreTx$ and setting $\varPreTx.\varSignature \opAssign \cnstEmptySet$.
        As in the previous proof we need to replace $\varPreTx.\varSigContext$ with a partially setup $\funStarAlt{\varSigContext}$.
        The simulator can achieve this by choosing ($\varSecKeyAlice$, ${\varSecKeyBob}_2$), ($\varNonceAlice$, ${\varNonceBob}_2$) such that:
        \begin{gather*}
        {\varPubKeyBob}_1 \opEqNoQ \varTx.\varSigContext.\varPubKey \opAddPoint {\funGen{\varSecKeyAlice + {\varSecKeyBob}_2}}^{-1} \\
        {\varRandBob}_1 \opEqNoQ \varTx.\varSigContext.\varRand \opAddPoint {\funGen{\varNonceAlice + {\varNonceBob}_2}}^{-1}
        \end{gather*}
        and then setting
        \[ \funStarAlt{\varSigContext} \opAssign \{ \varPubKey \opAssign \funGen{\varSecKeyAlice} \opAddPoint {\varPubKeyBob}_1, \varRand \opAssign \funGen{\varNonceAlice} \opAddPoint {\varRandBob}_1 \} \]
        \item Now finally $\cnstSimulator$ returns the outputs ($\varPreTx$, $\funStar{\varSpendableCoinAlice}$, ($\varSecKeyAlice$, $\varNonceAlice$)) to $\cnstAdversary$ (calling $\procDSendCoinsId$).
        \item The simulator now needs to construct a partially signed $\funStarAlt{\varPreTx}$, that is the partial signature must be valid under $\funGen{{\varSecKeyBob}_2}$ and nonce commitment $\funGen{\varNonceBob_2}$ and complete into $\varTx.\varSignature$ by adding the second partial signature valid under the combination of the remaing two keys.
        $\cnstSimulator$ can achieve this by running $\varSigBob \opFunResult \procSignPrt{\varTx.\varMsg}{\varSecKeyBob}{\varNonceBob}{\varTx.\varSigContext}$.
        He then clones $\varTx$ into $\funStarAlt{\varPreTx}$ and sets $\funStarAlt{\varPreTx}.\varSignature \opAssign \varSigBob$.
        Eventually $\cnstSimulator$ sends $\funStarAlt{\varPreTx}$ as if coming from Bob.
        \item When $\cnstAdversary$ calls $\procDFinTxId$ $\cnstSimulator$ checks if the $\funStar{\varSecKeyAlice}, \funStar{\varNonceAlice}$ passed as input parameters matches with $\varSecKeyAlice, \varNonceAlice$ that he returned in step 3 and returns $\varTx$ as well as sending $\cnstContinue$ to the TTP.
        If they don't match he sends $\cnstAbort$ to the TTP and outputs $\cnstFalsum$.
    \end{enumerate}

    The argument that the transcript produced by $\cnstSimulator$ is indistinguishable from a real one in the case that Alice is corrupted can be made as follows:
    If $\cnstAdversary$ tries to cheat and send invalid values to $\procDFinTxId$ the simulation will halt with $\cnstSimulator$ outputting $\cnstFalsum$, which is identical to what would happen in a real execution.
    The argument why $\varPreTx$ and $\funStarAlt{\varPreTx}$ are indistinguishable from real ones is similar to the argument we have already made in the last proof.
    $\varSecKeyAlice, {\varSecKeyBob}_2$ (as well as the nonce values) are uniformly distributed in $\cnstIntegersPrimeWithoutZero{\varPrime}$, which is the same during a real execution and therefore does not give $\cnstAdversary$ any chance to know that he is in a simulation.
    Furthermore by the way $\varSecKeyAlice$ and $\varPubKeyBob_1$ are constructed, the signature valid under $\funGen{\varSecKeyBob}_2$ must complete to $\varTx.\varSignature$ when the second part is added, which also makes $\funStarAlt{\varPreTx}$ indistinguishable from a real one.

    \textbf{Bob is corrupted}: Simulator $\cnstSimulator$ works as follows:
    \begin{enumerate}
        \item $\cnstSimulator$ invokes $\cnstAdversary$ and save $\funArray{\varPtSpendableCoinBob}$, $\varFundValue$ and $\varTime$ upon $\cnstAdversary$ calling $\procDSendCoinsId$.
        \item The simulator externally sends the inputs ($\funArray{\varPtSpendableCoinBob}$, $\varFundValue$, $\varTime$) to the TTP and receives the outputs ($\varTx$, $\funStar{\varSpendableCoinBob}$).
        Similar to the case that Alice was corrupted $\cnstSimulator$ chooses values ${\varSecKeyBob}_1, {\varNonceBob}_1$ as follows
        \begin{gather}
            \varPubKeyBob_2 \opEqNoQ \varTx.\varSigContext.\varPubKey \opAddPoint {\funGen{\varSecKeyAlice, {\varSecKeyBob}_1}}^{-1} \\
            \varRandBob_2 \opEqNoQ \varTx.\varSigContext.\varRand \opAddPoint {\funGen{\varNonceAlice, {\varNonceBob}}_1}^{-1}
        \end{gather}
        He then constructs $\varPreTx$ by cloning $\varTx$ removing the last output coin and proof, setting the signature field to $\cnstEmptySet$ and replacing the signature context with \[ \funStarAlt{\varSigContext} \opAssign \{ \varPubKey \opAssign \funGen{\varSecKeyAlice \opAddScalar {\varSecKeyBob}_1}, \varRand \opAssign \funGen{\varNonceAlice \opAddScalar {\varNonceBob}_1} \} \]
        Finally he returns ($\varPreTx$, ($\varSecKeyBob, \varNonceBob$)) to $\cnstAdversary$ (calling $\procDRecvCoinsId$).
        \item $\cnstAdversary$ now sends $\funStarAlt{\varPreTx}$ which $\cnstSimulator$
    \end{enumerate}
\end{proof}