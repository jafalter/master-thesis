...
\begin{proof}
    ...
    For \emph{Transaction Scheme Correctness} to hold $\procLedgr{\varLedger}{\varTx}$ must return an updated ledger $\funStar{\varLedger}$ with overwhealming probability for the two parties creating the transaction $\varTx$ in the following three steps:
    \begin{enumerate}
    ...
    \end{enumerate}
    Here are the requirements for $\procLedgr{\varLedger, \varTx}$ to return an updated ledger $\funStar{\varLedger}$ taken from~\cite{fuchsbauer2019aggregate}.
    \begin{enumerate}
        \item The current ledger $\varLedger$ passed as input must be valid (empty inputset) and the inputs used in $\varTx$ must be valid outputs in $\varLedger$.
