...
\begin{proof}
    ...
    We find the following three phases:
    ...
    \textbf{Phase 3}: Alice calls $\procZKfId{\cnstRelation_2}$ as the verifier and sends back the final transaction $\varTx$ to Bob which she then outputs.
    ...
    \begin{itemize}
        ...
        $\cnstSimulator$ can read $\varSigFin$ from the transaction in the output he received previously, he can further calculate the adversaries signature because he knows their signing secrets.
        From those two values he can then compute the value that $\varSigBob$ must have such that it will be completed to $\varSigFin$ when added to Alice's part of the signature.
        All the other values $\cnstSimulator$ needs to build $\funStarAlt{\varPreTx}$ he can simply read from the final transaction $\varTx$.
    ...
    \begin{enumerate}
        ...
        \item When $\cnstAdversary$ calls $\procZKfId{\cnstRelation_1},\procZKfId{\cnstRelation_2}$ as the verifier $\cnstSimulator$ simply checks equality we the values he sent and returns either 0 or one.
        ...
        $\cnstSimulator$ verifies the output coin added by $\cnstAdversary$ matches with $\funStarAlt{\varSpendableCoinBob}$, if it does not he sends $\cnstAbort$ to the TTP and outputs $\cnstFalsum$.
        Now $\cnstSimulator$ computes the following values from the signature context $\varSigContext$ provided in the final transaction and $\funStarAlt{\varSigContext}$ provided by $\cnstAdversary$:
        \begin{gather*}
        ...
        \begin{gather*}
            \varSigContext \opAssign \{ \varPubKey \opEqNoQ 1, \varRand \opEqNoQ \} \\
            \varSigContext \procSetupCtx{\varSigContext}{\varPubKeyAlice}{\varRandAlice}
        \end{gather*}
        And again send the pre-transaction to $\cnstAdversary$ as if coming from Alice.
        \item The simulator repeats the steps until the part where he rewinded earlier, instead of rewinding $\cnstSimulator$ now sends $\varTx$ as if coming from Alice and ouputs $\varTx$.
    \end{enumerate}

    Again we now claim that the simulation is indistinguishable from a real execution in all three phases.
    Note that due to the rewind we need to consider both the message sent both before and after the rewind.

    \begin{itemize}
        \item \textit{Phase 1:} In the first iteration the simulator chooses the input values $\funArray{\varSpendableCoin}$ and transaction value $\varFundValue$ and construct the pre-transaction using those chosen value rather than the real ones.
        We claim that the adversary can not extract to which value the input and output coins commit. (Except with neglible probability)
        ...
        Not being able to extract the coin values the adversary has no chance of knowing if they are correctly chosen at this point.
        ...
        However, as this time the pre-transaction is constructed from the real value which $\cnstSimulator$ received from the TTP, the pre-transaction is in fact identical to the pre-transaction sent in the real execution.
        ...
        For instance inputs which are spend in the final transaction where not present in the pre-transaction.
