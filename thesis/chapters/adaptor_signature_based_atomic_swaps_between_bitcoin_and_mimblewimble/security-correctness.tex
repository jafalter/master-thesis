...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item The simulator repeats the steps until 5. where he rewinded earlier, now instead of rewinding $\cnstSimulator$ sends $\cnstContinue$ to the TTP and sends $\varTx$ as if coming from Alice, and finally outputs $\varTx$.
    \end{enumerate}

    Again we now claim that the simulation is indistinguishable from a real execution in all three phases.
    Note that due to the rewind we need to consider both the message sent before and after the rewind.
    ...
    \begin{itemize}
        ...
        We claim that the adversary cannot distinguish the chose from the real coin commitments (Except with neglible probability).
