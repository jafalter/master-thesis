...
\begin{varwidth}{\textwidth}
    ...
    \< \< \< \< \cdots \\
    \< \< \< \< \procZkf{\cnstRelation_1}{(\varSecKeyCarol, \funGen{\varSecKeyCarol})} \\
    \< \sendmessageleft*{\varPreTx, \funGen{\varSecKeyCarol}} \< \\
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Otherwise he sends $(\varPreTx, \varFundValue)$ to the TTP computing $\procRecvCoinsId$ and receives the outputs $(\varPreTx, \funStar{\varPtSpendableCoinBob})$.
        \item $\cnstSimulator$ proceeds by taking the last output $\funStarAlt{\varCoinShared}$ from $\varPreTx$ (received from the TTP) and computes $\funGen{\varSecKeyCarol} \opAssign \funStarAlt{\varCoinShared} \opAddPoint {\varCoinShared}^{-1}$.
        The simulator now sends $\varPreTx$ (as received from the TTP) and $\funGen{\varSecKeyCarol}$ to $\cnstAdversary$ as if coming from Carol and sends $\cnstContinue$ to the TTP.
        ...
        \item When the adversary calls $\procDRProofId$ the simulator saves $\varSecKeyBob$ to its memory and returns the last element of $\varPreTx.\varProofs$ as received from the TTP.
        \item For the the call to $\procDSignId$ the simulator returns the $\varPreTx.\varSignature$ as the signature and $\funGen{\varSecKeyBob} \opAddPoint \funGen{\varSecKeyCarol}$ as the public key.
    ...
    \begin{itemize}
        ...
        The simulator can then send the updated pre-transaction as received from the TTP and the extracted real value for $\funGen{\varSecKeyCarol}$.
    ...
    \begin{enumerate}
        \item Since Carol does not have any inputs in this protocol $\cnstSimulator$ can simply send $\cnstEmptySet$ to the TTP and receives ($\varPreTx, \funStar{\varSpendableCoinCarol}$) from which he extracts Carols bliding factor (and secret key) as $\varSecKeyCarol \opAssign \funStar{\varSpendableCoinCarol}.\varBlindingFactor$.
        He can now create the initial shared coin $\varCoinShared$ by taking the last output of $\varPreTx$ as $\funStarAlt{\varCoinShared}$ and calculating $\varCoinShared \opAssign \funStarAlt{\varCoinShared} \opAddPoint {\funGen{\varSecKeyCarol}}^{-1}$.
        he can further create the initial pre-transaction by removing the last entry of the output coin list, last entry of the proof list and signature from $\varPreTx$ received by the TTP.
        ...
        \item The adversary then sends the updated $\varPreTx$ which the simulator compares to the pre-transaction received from the TTP.
        If the two transaction are found not to match $\cnstSimulator$ will output $\cnstFalsum$ and send $\cnstAbort$ to the TTP halting the execution, otherwise he will send $\cnstContinue$.
        \item Upon the adversary calling $\procDRProofId$ the simulator will return the proof at the last position in the proofs array of $\varPreTx$ received from the TTP.
        \item The simulator then extracts $\varFundValue \opAssign \funStar{\varPtSpendableCoinCarol}$ and computes $\varPubKeyBob \opAssign \varCoinShared \opAddPoint {\funGenH{\varValue}}^{-1}$ and returns $\varPreTx.\varSignature$ and $\funStar{\varSecKeyCarol} \opAddPoint \varPubKeyBob$ when $\cnstAdversary$ calls $\procDSignId$.
