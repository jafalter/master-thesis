...
\begin{definition}[Mimblewimble Transaction Scheme]
    ...
    \begin{itemize}
        ...
        \item $\{1,0\} \opFunResult \procVerfTx{\varTx}$: The verification algorithm is taken from the Fuchsbauer et al. paper (there it is named $\styleFunction{MW.Ver}$) and can be used to verify mimblewimble transactions.
        If an invalid transaction is passed it will output 0, otherwise 1.
        The routine verifies four statements:
        \begin{enumerate}
            \item Every input and output coin only appears once in the transaction.
            \item The union of input and output coins is the empty set.
            \item The transaction signature verifies with the excess value of the transaction as public key, which is calculated by summing up to output coins and subtracting the input coins. (See~\ref{sec:pre:mimblewimble})
            \item For every output coin the range proof verifies.
        \end{enumerate}
    \end{itemize}
\end{definition}
...
\begin{definition}[Script Transaction Scheme Correctness]
