...
First of all we need to define the notion of a an aggregateable signature scheme:
...
\end{definition}
In the case of the Schnorr signature scheme we can aggregate signatures simply by concatenating the them like $\varSignature_1 \opConc \varSignature_2$.
...
Now as we have changed the nonce commitment $\varRand$ as well as the public key $\varPubKey_a$ in our aggregated signature the Schnorr challenge $\varSchnorrChallenge$ will be different from the one used by the individual signatures and thereby making the verification algorithm return 0.
...
By using the Two-Party Schnorr instead of the aggregated Schnorr we save space, as we only need to store one single signature instead of multiple.
...
In order to start the signing process the final composite $\varPubKey_a$ and nonce commitment $\varRand$ needs to be known.
That also entails that the flow pointed out in~\cite{fuchsbauer2019aggregate}, in which the transaction sender starts the signing process and the receiver completes it is now longer possible.
...
While having to add an additional round would seem like an inconvenience we discover that by doing so we avoid the following attack.

\paragraph{Stolen Transaction Attack} Imagine a sender Alice and receiver Bob.
...
However, an attacker is secretly listening on this channel and is able to see each message exchanged between the two.
With the notions defined by Fuchsbauer et al. Alice starts the protocol by running $\varPreTx \opFunResult \procFuchsSend$ and sending $\varPreTx$ to Bob via the channel.
...
Already containing Alice signature all the attacker has to do is guess the value that Alice might want to send, create and output coin with that value, add his own signature and broadcast the final transaction to the network.
Since the range of possible amounts that Alice might want to transfer being limited, it is trivial for the attacker to guess it.
When now Bob comes back to finalize the transaction, he will discover that he continue with the protocol as the input coins spend by Alice have already been spend and send to the attacker.

Starting the signing process only at the receivers turn and introducing a third round solves this issue.
Even if the attacker would be able to sniff the pre-transaction constructed by Bob, containing his partial signature, he is missing Alice's signature and is therefore not able to build a valid transaction.
