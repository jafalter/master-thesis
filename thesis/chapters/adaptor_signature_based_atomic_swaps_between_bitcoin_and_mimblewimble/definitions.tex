...
Finally we define a two-party protocol $\procDBuildMwTxId$ which takes as input a list of spendable coins $\funArray{\varSpendableCoin}$, a transaction value $\varFundValue$ and a optional timelock $\varTime$ and uses the functions defined earlier to output a valid transaction $\varTx$ as well as the newly spendable coins to both parties.
