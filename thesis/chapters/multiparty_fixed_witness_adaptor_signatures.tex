...
A two-party signature scheme wrt. a hard relation $\cnstRelation$ \todo[inline]{why a two-party signature scheme is wrt. a hard relation $\cnstRelation$?} is an extension of a signature scheme as defined in definition~\ref{def:signatureScheme}, which allows us to distribute signature generation for a composite public key shared between two parties Alice and Bob.
    Alice and Bob want to collaborate to generate a signature valid under the composite public key $\varPubKeyComp \opAssign \varPubKeyAlice + \varPubKeyBob$ \todo{this way of defining the public key as the composition of two public keys is not correct} without having to reveal their secret keys to each other.
    
    
    \todo[inline]{Pedro: I changed the definition of two-party signature scheme. Your version is below commented out}
...
\begin{proof} We first proof $\cnstaEUFCMA$ by providing a black box reduction from $\cnstaEUFCMA$ to $\cnstEUFCMA$.
