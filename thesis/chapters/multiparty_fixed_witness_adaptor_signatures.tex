...
\begin{definition}[Two Party Fixed Witness Adaptor Schnorr Signature Scheme]
    ...
    \begin{itemize}
        ...
        It outputs the partial pre signature, which can be verified to contain $\varWit$ by Bob (knowing $\varGWit$).
        \item $\procVrfAptId$ makes it possible to verify the validity of an partial pre signature plus that it indeed contains the secret witness of $\varGWit$. Assume Bob provided a partial pre signature to Alice,
    ...
    \fbox{
        \parbox{\textwidth} {
            \procedure[linenumbering, syntaxhighlight=auto]{$\procSetupApt{\varSecParam}$} {
                ...
                \varGWit \opAssign \funGen{\varWit} \\
...
\begin{definition}[Pre-signature correctness]\label{def:preSigCorrectness}
    Similiar to how it is defined in~\cite{aumayr2020bitcoinchannels} we additionally require our signature scheme to satisfy \textit{pre-signature correctness}. This property is given if a presignature
    is generated by $\procGenPtAptSigId$ can be completed into a final (partial) signature for all pairs $(\varWit \opSeperate \varGWit)$, from which it will
    be possible to extract the witness computing $\procExtWitId$ with the required parameters.\\
    More formally Pre-signature correctness if given if for every security parameter $\varN \in \cnstNatural$, message $\varMsg \in \cnstBinary{*}$, and every statement/witness pair
    $\varGWit \opSeperate \varWit)$ in a relation $\cnstRelation$ it must hold that:
    \condprob{
        \procVrfApt{\varMsg}{\varSecKeyAlice}{\varNonceAlice}{\varPubKeyBob}{\varRandBob}{\varSigBob}{\varGWit} \opEq 1
        \opAnd
        \procVerf{\varMsg}{\varSigFin}{\varPubKeyAlice \opAddPoint \varPubKeyBob} \opEq 1
        \opAnd
        (\varGWit \opSeperate \funStar{\varWit} \opIn \cnstRelation)
    }{
        (\varKeyPairAlice \opSeperate (\varNonceAlice \opSeperate \varRandAlice)) \opFunResult \procSetupPartSig{\varSecParam}
        (\varKeyPairBob \opSeperate (\varNonceBob \opSeperate \varRandBob) \opSeperate (\varWit \opSeperate \varGWit)) \opFunResult \procSetupApt{\varSecParam}
        \varSigAptBob \opFunResult \procGenPtAptSig{\varMsg}{\varSecKeyBob}{\varNonceBob}{\varPubKeyAlice}{\varRandAlice}{\varWit}
...
We have already proven that $\procVerf{\varMsg}{\varSigFin}{\varPubKeyAlice \opAddPoint \varPubKeyBob} \opEq 1$ holds in our instantiation of the signature scheme in the correctness proof \ref{prf:aptSchnorr}.
To show that in addition to correctness also pre-signature correctness holds we prove the remaining statements in proof~\ref{prf:aptSchnorrPreSigCorr}.
...
\begin{proof}
    ...
    \begin{gather}
        (\varKeyPairAlice \opSeperate (\varNonceAlice \opSeperate \varRandAlice)) \opFunResult \procSetupPartSig{\varSecParam} \\
        (\varKeyPairBob \opSeperate (\varNonceBob \opSeperate \varRandBob) \opSeperate (\varWit \opSeperate \varGWit)) \opFunResult \procSetupApt{\varSecParam} \\
        \varSigAptBob \opFunResult \procGenPtAptSig{\varMsg}{\varSecKeyBob}{\varNonceBob}{\varPubKeyAlice}{\varRandBob} \\
        \procVrfApt{\varMsg}{\varSecKeyAlice}{\varNonceAlice}{\varPubKeyBob}{\varRandBob}{\varGWit}{\varSigAptBob} \opEq 1
    \end{gather}
    The proof is by continous substitutions in the equation checked by the verifier:
    \begin{align}
        \funGen{\varSigAptBob} &\opEq \opPointScalar{\varRandBob}{\varSchnorrChallenge} \opAddPoint \varPubKeyBob \opAddPoint \varGWit \\
        ...
        \opPointScalar{\varRandBob}{\varSchnorrChallenge} \opAddPoint \varPubKeyBob \opAddPoint \varGWit &\opEq \opPointScalar{\varRandBob}{\varSchnorrChallenge} \opAddPoint \varPubKeyBob \opAddPoint \varGWit \\
        1 &\opEq 1
    \end{align}
\end{proof}
