    ...
    \fbox{
        \parbox{\textwidth} {
            \procedure[linenumbering, syntaxhighlight=auto]{$\procSetupApt{\varSecParam}$} {
                ...
                \pcreturn (\varKeyPair, (\varNonce, \varRand), (\varWit, \varStatement))
            }
...
\begin{definition}[$\cnstaEUFCMA$]\label{def:aeufcma}
        Additionally to the reuglar definition of existential unforgeability under chosen message attacks we additionally require that it is hard to produce a forged partial signature $\varSigPart$ if the
    ...
    For the definition of $\cnstaEUFCMA$-security we define the experiment $\procExpForgId$ for a $\cnstPolyTime$ adversary $\cnstAdversary$ and a adapted signature scheme $\varSigSchemeApt$ as
    follows:\\
    \fbox{
        \parbox{\textwidth}{
            \procedure[linenumbering, syntaxhighlight=auto]{$\procExpForg{\varN}$} {
                ( (\varSecKeyAdv \opSeperate \varPubKeyAdv) \opSeperate (\varNonceAdv \opSeperate \varRandAdv) ) \opFunResult \procSetupPartSig{\varSecParam} \\
                \varMsg \opFunResult \cnstAdversary^{\procSignOracleId}(\varPubKeyAdv) \\
                (\varSigPartApt \opSeperate \varStatement) \opFunResult \procSignOracle{\varMsg}{\varPubKeyAdv}{\varRandAdv} \\
                \varSigPart \opFunResult \cnstAdversary^{\procSignOracleId}(\varSigPartApt \opSeperate \varStatement) \\
                \varSigFin \opFunResult \procFinOracle{\varSigPart}{\varRandAdv} \\
                \pcreturn \procVerf{\varMsg}{\varSigFin}{\varPubKeyAdv \opAddPoint \varPubKey}
            }\\[2\baselineskip]
            \procedure[linenumbering, syntaxhighlight=auto]{$\procSignOracle{\varMsg}{\varPubKeyAdv}{\varRandAdv}$} {
                (\varKeyPair \opSeperate (\varNonce \opSeperate \varRand) \opSeperate (\varWit \opSeperate \varStatement) \opFunResult ) \opFunResult \procSetupApt{\varSecParam} \\
                \pcreturn \procGenPtAptSig{\varMsg}{\varSecKey}{\varNonce}{\varPubKeyAdv}{\varRandAdv}{\varWit}
            }\\[2\baselineskip]
            \procedure[linenumbering, syntaxhighlight=auto]{$\procFinOracle{\varSigPart}{\funStar{\varRand}}$} {
                \pcreturn \opFunResult \procFinAptSig{\varSigPart}{\varSigPartApt}{\varRand}{\funStar{\varRand}}{\varWit}
            }
        }
    }
\end{definition}
