...
which is specifically tailored for the use in an Atomic Swap scenario in which (at least one side of the swap) uses a two-party protocol to generate transaction signatures and keypairs are fixed beforehand. \todo[inline]{Pedro: I think the main difference is that in this version of the adaptor signature, one party knows the ``secret'' part of the adaptor}
...
\todo[inline]{Pedro: In the definition below, you are mixing the intuition (necessary but outside of the definition) and the definition itself.}
\begin{definition}[Two Party Signature Scheme]
    ...
    \todo[inline]{Pedro: Add input and output to the methods}
    \begin{itemize}
...
\begin{definition}[Two Party Fixed Witness Adaptor Schnorr Signature Scheme]
    ...
    \todo[inline]{Pedro: add the input and output for each method. For instance, the notion of ``partial signature'' should be clearer from previous definition.}
    \begin{itemize}
...
\begin{definition}[Secure Adaptor Signature Scheme]
    As defined by Aumayr et al. in~\cite{aumayr2020bitcoinchannels}, a secure adaptor signature scheme needs four\todo{Pedro: three?} security properties to be fulfilled:
    \todo[inline]{Pedro: correctness is not a security property}
    \begin{enumerate}
...
Similar to how it is defined in~\cite{aumayr2020bitcoinchannels} additionally to \textit{correctness}\todo{Pedro: you might want to add a link where you define this} we require our signature scheme to satisfy \cnstAptSigCorrectness.
...
More formally \cnstAptSigCorrectness is given if for every security parameter $\varN \in \cnstNatural$, message $\varMsg \in \cnstBinary{*}$, keypairs $\varKeyPairAlice \opSeperate \varKeyPairBob$ \todo{Pedro: how are they generated? with KeyGen?}
...
\begin{center}
\todo[inline]{Pedro: here $\procSignPt{x}{y}{z}$ is a protocol, not an algorithm right? I think there are missing steps: For instance, where is $sk_B$ coming from in step 4? Shouldn't attacker know $X$ also in step 6? We should go over it again looking at definition in \url{https://eprint.iacr.org/2020/476.pdf} paper} 
    \fbox{
    \begin{varwidth}{\textwidth}
        \procedure[linenumbering]{$\procExpForg{\varN}$} {
        \varSet \opAssign \cnstEmptySet \\
        \varKeyPairBob \opFunResult \procSetup{\varSecParam} \\
        (\varMsg \opSeperate \varSecKeyAdv) \opFunResult \cnstAdversary^{\procSignOracle{\cdot}{\cdot}}(\varPubKeyBob) \\
        (\varWit \opSeperate \varStatement) \opFunResult \procGenR{\varSecParam} \\
        ...
        \pcreturn (\varMsg \opNotIn \varSet \opAnd \procVerf{\varMsg}{\varSigFin}{\varPubKeyAdv \opAddPoint \varPubKeyBob})
        }\\[2\baselineskip]
        \procedure[linenumbering]{$\procSignOracle{\varMsg}{\varSecKeyAdv}$} {
...
\begin{definition}[\cnstWitnessExtractability]
	...
	\todo[inline]{Pedro: as before, here the oracle run a single party function for $\procSignPt{x}{y}{z}$}
    \begin{center}
        \fbox{
        \begin{varwidth}{\textwidth}
...
\begin{figure}
    ...
    \end{center}
    \todo[inline]{Pedro: minor mistake in verify?}
